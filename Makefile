...
IMAGE := ''
ifeq ($(IMAGE),slim)
	SUPER_LINTER_TEST_CONTINER_URL := "ghcr.io/bcdonadio/bcd-super-linter:slim"
	IMAGE := "slim"
else
	SUPER_LINTER_TEST_CONTINER_URL := "ghcr.io/bcdonadio/bcd-super-linter:standard"
	IMAGE := "standard"
endif

.PHONY: inspec
inspec: inspec-check ## Run InSpec tests
	LOCAL_IMAGE="$$(docker images $(SUPER_LINTER_TEST_CONTINER_URL) |grep 'ghcr.io/bcdonadio/bcd-super-linter')"; \
...
docker:
	...
	DOCKER_BUILDKIT=1 docker buildx build --load \
		...
		-t ghcr.io/bcdonadio/bcd-super-linter .
