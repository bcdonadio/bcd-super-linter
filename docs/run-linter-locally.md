...
- Pull the latest **Docker** container down from **DockerHub**
  - `docker pull bcdonadio/bcd-super-linter:latest`
...
- You can run the container locally with the following **Base** flags to run your code:
  - `docker run -e RUN_LOCAL=true -e USE_FIND_ALGORITHM=true -v /path/to/local/codebase:/tmp/lint bcdonadio/bcd-super-linter`
    - To run against a single file you can use: `docker run -e RUN_LOCAL=true -e USE_FIND_ALGORITHM=true -v /path/to/local/codebase/file:/tmp/lint/file bcdonadio/bcd-super-linter`
...
### .bcdonadio/bcd-super-linter.env
...
docker run --rm \
    -e RUN_LOCAL=true \
    --env-file ".bcdonadio/bcd-super-linter.env" \
    -v "$PWD":/tmp/lint bcdonadio/bcd-super-linter:v5
```
...
jobs:
  lint:
    ...
    steps:
      ...
      - run: cat .bcdonadio/bcd-super-linter.env >> "$GITHUB_ENV"
      - name: Lint Code Base
...
- `docker run -it --entrypoint /bin/bash bcdonadio/bcd-super-linter`
- This will drop you in the command-line of the docker container for any testing or troubleshooting that may be needed.

### Found issues

If you find a _bug_ or _issue_, please open a **GitHub** issue at: [bcdonadio/bcd-super-linter/issues](https://github.com/bcdonadio/bcd-super-linter/issues)
